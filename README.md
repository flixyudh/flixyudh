# Hi there, I'm Yudi Iswandi! 👋
[![linkedin](https://skillicons.dev/icons?i=linkedin)](https://www.linkedin.com/in/yudiiswandi)
[![linkedin](https://skillicons.dev/icons?i=gmail)](mailto:yudhi.trendhi@gmail.com)

**React Native Engineer | Golang | Typescript | Hybrid Mobile App Specialist**

I am an experienced React Native Engineer with over **6 years of expertise** in building robust hybrid mobile applications. Currently based in Tangerang, Banten, I specialize in full-cycle application development, system optimization, and leading technical teams to deliver high-quality software solutions.

---

### 🛠️ Apps I'm Building

#### 🚀 [Fams](https://fams.yudimy.id) (Flix App Management System)
A full-stack **App Distribution & CodePush Management Dashboard** — a self-hosted alternative to Microsoft AppCenter. It allows teams to:

- Distribute iOS (`.ipa`) and Android (`.apk`) builds to testers via download links and QR codes
- Push OTA JavaScript updates via **CodePush** without App Store resubmission
- Manage organizations, team members, roles, and invitations
- Monitor release activity via a rich analytics dashboard

* **Frontend:** React, TypeScript, and RSBuild for optimized build speeds.
* **Backend:** High-concurrency services powered by **Golang**.

---

### 🚀 About Me

- 🔭 I’m currently working as a **React Native Engineer** at **PT. Omni Digitama Internusa (Kawan Lama Group)**.
- 📱 I have a strong track record in **React Native**, **JavaScript/Typescript/ES6**, and **Mobile Architecture**.
- 🤝 I enjoy collaborating across departments to ensure technical feasibility of UI/UX designs.
- ⚡ **Soft Skills:** Problem Solving, Business Flow Analysis, Team Leadership, R&D, Debugging.

---

### 🛠️ Tech Stack & Tools

**Languages**
[![language](https://skillicons.dev/icons?i=js,ts,java,go)](https://github.com/flixyudh)

**Frameworks & Libraries**
[![framwork](https://skillicons.dev/icons?i=react,nodejs,express,androidstudio)](https://github.com/flixyudh)

**Tools & Platform**
[![tools](https://skillicons.dev/icons?i=git,vscode,figma,bitbucket,docker,postman)](https://github.com/flixyudh)

---

### 💼 Work Experience

**React Native Engineer** @ *PT. Omni Digitama Internusa (Kawan Lama Group)* *(Dec 2021 - Present)*
- Ensure technical feasibility of UI/UX designs.
- Improve system performance, scalability, and reliability.
- Coordinate and supervise the product development team.

**Mobile Developer** @ *PT. Summarecon Agung Tbk.* *(Feb 2020 - Nov 2021)*
- Implemented Figma/Zeplin mockups into robust code.
- Ensured applications ran according to specific business processes.
- Maintained application stability and bug fixing.

---

### 📊 GitHub Stats

<div align="center">
  <img src="https://streak-stats.demolab.com?user=flixyudh&locale=en&mode=daily&theme=dracula&hide_border=false&border_radius=5&order=3" height="150" alt="streak graph"  />
  <img src="https://github-profile-trophy.vercel.app?username=flixyudh&theme=dracula&column=-1&row=1&margin-w=8&margin-h=8&no-bg=false&no-frame=false&order=4" height="150" alt="trophy graph"  />
</div>
